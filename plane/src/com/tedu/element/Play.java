...
//	变量专门用来记录当前主角面向的方向,默认为是up
	...
	private static boolean pauseGame = false;  // true暂停游戏，false继续
...
//		玩家血条
		...
		g.drawRect(10, 530, 208, 19);
		g.fillRect(14, 534, 4*this.getHp(), 11);
		
//		g.setColor(Color.RED);
//		g.setFont(new Font("仿宋",Font.BOLD,30));     //字体，风格，字号
//		g.drawString("你的血量: " + Integer.toString(this.getHp()), 300, 50);
	}
	/*
...
//	}
	...
	public void move() {
		if (this.left && this.getX()>0) {
			this.setX(this.getX() - this.getSd());
		}
		if (this.up  && this.getY()>0) {
			this.setY(this.getY() - this.getSd());
		}
		if (this.right && this.getX()<580-this.getW()) {  //坐标的跳转由大家来完成
			this.setX(this.getX() + this.getSd());
		}
...
//	}
	...
	private long fileTime=0;
//  fileTime和传入时间gameTime进行比较，赋值等操作，控制子弹间隔；
	@Override
	protected void add(long gameTime) {
//		构造一个类要做比较多的工作，可以使用小工厂
//      将构造对象的多个步骤进行封装成一个方法
//      传递一个固定格式（x:3,y:5,fx:up）
		if(!pkType) {
			return;
		}
		if(gameTime-fileTime < 100) {
			return;
		}
		fileTime = gameTime;
		ElementObj obj = GameLoad.getObj("file");
		ElementObj element = obj.createElement(this.toString());//以后框架学习会碰到
//		会帮助你返回对象的实体，拿到初始化数据
...
//		学习技术不犯法，但是不要用技术做犯法的事.
	...
	public static void setPauseGame(boolean isleap) {
		pauseGame = isleap;
	}
