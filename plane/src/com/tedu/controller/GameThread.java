...
//					System.out.println(listB);
	...
	public void PlayCrushEnemy() {
		for(int i = 0; i < em.getElementsByKey(GameElement.PLAY).size(); i++) {
			ElementObj play = em.getElementsByKey(GameElement.PLAY).get(i);
			for(int j = 0; j < em.getElementsByKey(GameElement.ENEMY).size(); j++) {
				ElementObj enemy = em.getElementsByKey(GameElement.ENEMY).get(j);
				if(play.pk(enemy)) {
					play.setHp(play.getHp() - 5);
					if(play.getHp() < 0)
						play.setLive(false);
					enemy.setLive(false);
					break;
