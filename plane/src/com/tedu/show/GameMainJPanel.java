...
//	联动管理器
	...
	/**
	 * paint方法是进行绘画元素。 绘画时是有固定的顺序，先绘画的图片会在底层，后绘画的图片会覆盖先绘画的 约定：本方法只执行一次,想实时刷新需要使用 多线程
	 */
...
//		System.out.println("enemy:" + enemyAppear + ",boss:" + bossAppear);
//		System.out.println(em.getElementsByKey(GameElement.BOSS).isEmpty());
		if (!em.getElementsByKey(GameElement.ENEMY).isEmpty())
			enemyAppear = true;
		if (!em.getElementsByKey(GameElement.BOSS).isEmpty())
			bossAppear = true;
		if (enemyAppear && bossAppear || !enemyAppear && !bossAppear)
			staticAppear = true;
		else
			staticAppear = false;
		if (staticAppear && em.getElementsByKey(GameElement.BOSS).isEmpty()) {
			em.init();
			g.setColor(Color.RED);
			g.setFont(new Font("微软雅黑", Font.BOLD, 50));
			g.drawString("恭喜你通过本关！！", 50, 300);
			init();
