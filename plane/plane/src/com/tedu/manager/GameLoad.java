...
//				就可以自动的创建和加载 我们的地图啦 
	...
	//加载背景图片
	public static void loadBackground(int flag) {
		loadObj();
		ElementObj obj=getObj("background");  
		ElementObj background = obj.createElement(Integer.toString(flag));
//		ElementObj play = new Play().createElement(playStr);
//		解耦,降低代码和代码之间的耦合度 可以直接通过 接口或者是抽象父类就可以获取到实体对象
		em.addElement(background, GameElement.BC);
	}
	 ...
	 */
	public static void loadImg(int flag) {//可以带参数，因为不同的关也可能需要不一样的图片资源
...
//			System.out.println(texts);
	 ...
	 */
//		不同关卡飞机不一样
	public static void loadPlay(int flag) {
//		System.out.println("loadplay");
		loadObj();
		String playStr="500,500," + Integer.toString(flag);
...
//		加载boss
	public static void loadBoss(int flag) {
		loadObj();
		
		ElementObj obj=getObj("boss");  
		ElementObj play = obj.createElement("300,60,boss"+Integer.toString(flag));
//		ElementObj play = new Play().createElement(playStr);
//		解耦,降低代码和代码之间的耦合度 可以直接通过 接口或者是抽象父类就可以获取到实体对象
		em.addElement(play, GameElement.BOSS);
	}
...
//		加载敌人
	public static void loadEnemy(int s) {
		loadObj();
//		String enemyStr="";
//		int num = 0;
//		ElementObj obj = getObj("enemy");
//		ElementObj enemy = obj.createElement(enemyStr);
//		em.addElement(GameElement.ENEMY, enemy);
		for(int i = 0; i < 5; i++) {
			ElementObj obj = getObj("enemy");
			ElementObj enemy = obj.createElement(Integer.toString(s));
